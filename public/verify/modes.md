> Source: https://docs.pollus.tech/verify#modes
...
## Agent Quick-Start
- Source URL: https://docs.pollus.tech/verify#modes
- Credentials / env vars needed: none to choose a mode (each mode later needs an App API key from https://dev.pollus.tech; Hosted also needs a `workflow_id`)
...
[Account (Managed by Valyd)](https://docs.pollus.tech/verify/managed.md).
...
IF you need an end-user KYC flow with live camera/ID + selfie capture, and you do NOT want to build the capture UI:
  ...
  → Next: create a Workflow (get workflow_id) and configure a webhook in the Console (https://docs.pollus.tech/verify#console), then create a session (https://docs.pollus.tech/verify#quickstart).
...
IF you want to call individual capabilities server-to-server from your backend with your own UI and data, and want a synchronous result:
  ...
  → Next: get an App API key from the Console (https://docs.pollus.tech/verify#console), then call the endpoint (https://docs.pollus.tech/verify#quickstart).
